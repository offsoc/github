...
|}")
  ...
  test "disabled support for google charts backed math tag" do
    wiki = WikiParser.new(:data => "<math>1+1=2</math>", :math_formatter => :google)
    data = wiki.to_html
    assert_equal "\n<p>&lt;math&gt;1+1=2&lt;/math&gt;</p>", data
  end
...
EOS
  ...
  test "decreasing header levels" do
    wiki = WikiCloth::WikiCloth.new({:data => "===CCC===\nccc\n==BBB==\nbbb\n=AAA=\naaa\n"})
    data = wiki.render
    assert data =~ /<h3>.*CCC.*ccc.*<h2>.*BBB.*bbb.*<h1>.*AAA.*aaa/m
  end

  test "don't parse incomplete headings incorrectly" do
    wiki = WikiCloth::WikiCloth.new({:data => "===C\n"})
    data = wiki.render
    assert data !~ /<h\d>/
  end

  test "no switches in HTML" do
    wiki = WikiCloth::WikiCloth.new(data: "<pre>\ndef __init__(self):\n</pre>\n")
    assert_equal "\n<p><pre>def &#95;&#95;init&#95;&#95;(self)&#58;\n</pre></p>", wiki.render
  end
end
