...
// Clones tags, etc. of a node, but does not copy the content.  The
// clone shares no structure with the original node: all owned strings and
// values are fresh copies.
GumboNode* clone_node(GumboNode* node, GumboParseFlags reason) {
  assert(node->type == GUMBO_NODE_ELEMENT || node->type == GUMBO_NODE_TEMPLATE);
  ...
  *new_node = *node;
  ...
  // Here we deviate from the 8.2.3.3 HTML5 spec, and from upstream gumbo-parser.
  // The spec says we should duplicate all attributes of the node, but that leaves us
  // open to a DoS attack from payloads with arbitrarily long attributes in unclosed
  // formatting tags. Following the spec would cause us to duplicate those attributes,
  // expanding a small string into many megabytes of HTML content.
  // Instead, we remove all attributes when cloning an unclosed formatting tag.
  // See https://github.com/github/heart-services/issues/1262 for more details.
  gumbo_vector_init(0, &element->attributes);
  return new_node;
}
