...
class WikiBuffer::Var < WikiBuffer
  ...
  def to_html
    ...
    if self.is_function?
      ...
      stringified_params = params.map { |p| p.to_s.strip }
      ret = default_functions(function_name, stringified_params)
      ret ||= @options[:link_handler].function(function_name, stringified_params)
      ret.to_s
    elsif self.is_param?
      ret = nil
      ...
      if template_stack.last == params[0]
        ...
        ret_template = @options[:link_handler].include_resource(key,key_options)
        ret = ret_template.nil? ? +"" : ret_template.to_s
