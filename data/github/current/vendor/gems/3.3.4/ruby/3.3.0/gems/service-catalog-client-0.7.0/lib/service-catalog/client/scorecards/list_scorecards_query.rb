...
module ServiceCatalog
  class Client
    module Scorecards
      ...
      module ListScorecardsQuery
        # The instrumentation key for this query used by ActiveSupport::Notifications.
        INSTRUMENTATION_KEY = "service_catalog_client.scorecards.scorecards"
        ...
        def self.execute(client:, context: {}, detail_fragment: ScorecardsQueryBinder::SCORECARD_DETAIL_FRAGMENT, **kwargs)
          ...
          ActiveSupport::Notifications.instrument(INSTRUMENTATION_KEY) do
            loop do
              ...
              response["data"]["scorecards"]["edges"].each do |edge|
                data["data"]["scorecards"] << edge["node"]
              end
              break if data["data"]["scorecards"].size >= response["data"]["scorecards"]["totalCount"]
              variables["after"] = response["data"]["scorecards"]["edges"].last["cursor"]
            end
          end
        ...
        def self.build_query(variables, detail_fragment: ScorecardsQueryBinder::SCORECARD_DETAIL_FRAGMENT, **kwargs)
          <<~GRAPHQL
            query listScorecards#{GraphqlQuerySemantics.graphql_variable_declaration(variables, variable_types: VARIABLE_GRAPHQL_TYPES, include_parens: true)} {
              scorecards#{GraphqlQuerySemantics.graphql_variable_passing(variables, include_parens: true)} {
                totalCount
                edges {
                  cursor
                  node {
    ...
    class ScorecardsQueryBinder < QueryBinder
      ...
      # @return [Response]
      def list(**kwargs)
        Response.new Client::Scorecards::ListScorecardsQuery.execute(client: client, **kwargs)
      end
    end
  end
end
