...
bt_start () {
  if [ -z "$BT_DISABLED" -o "$BT_DISABLED" = "0" ]; then
    local caller="$(basename ${BASH_SOURCE[1]} 2>/dev/null):${BASH_LINENO[0]}"
    local desc_checksum=$(printf "%s" "$@" | sha256sum | awk '{print $1}')
    local timestamp=$(bt_timestamp)
...
bt_end () {
  if [ -z "$BT_DISABLED" -o "$BT_DISABLED" = "0" ]; then
    local caller="$(basename ${BASH_SOURCE[1]} 2>/dev/null):${BASH_LINENO[0]}"
    local desc_checksum=$(echo -n "$@" | sha256sum | awk '{print $1}')
    echo "$(bt_timestamp) $caller $1" >> /tmp/bt.$desc_checksum
  fi
}
export -f bt_end
