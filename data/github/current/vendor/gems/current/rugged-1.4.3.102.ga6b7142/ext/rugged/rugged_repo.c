 ...
 *
 *  :initial_branch::
 *    The desired name of the default branch (eg., 'main')
 *
...
{
	git_repository *repo = NULL;
	VALUE rb_path, rb_is_bare, rb_options, rb_initial_branch = Qnil;
	int error;
	...
	if (!NIL_P(rb_options)) {
		/* Check for `:backend` and `:initial_branch` */
		VALUE rb_backend = rb_hash_aref(rb_options, CSTR2SYM("backend"));
		rb_initial_branch = rb_hash_aref(rb_options, CSTR2SYM("initial_branch"));
	...
	if (!NIL_P(rb_initial_branch)) {
		Check_Type(rb_initial_branch, T_STRING);
		opts.initial_head = StringValueCStr(rb_initial_branch);
	}
