...
import InboxDefaultViewRequest, {
  type InboxDefaultViewV1Query,
  type InboxDefaultViewV1Query$data,
} from './__generated__/InboxDefaultViewV1Query.graphql'
...
type InboxDefaultViewRendererProps = {
  queryReference: PreloadedQuery<InboxDefaultViewV1Query>
}

function getOwner(data: InboxDefaultViewV1Query$data | null) {
...
function getState(data: InboxDefaultViewV1Query$data | null) {
  if (!data || !data.node) return null
...
function getTitle(data: InboxDefaultViewV1Query$data | null) {
  if (!data || !data.node) return null
...
function getNumber(data: InboxDefaultViewV1Query$data | null) {
  if (!data || !data.node) return null
...
function getAuthor(data: InboxDefaultViewV1Query$data | null) {
  if (!data || !data.node) return null
...
function getBody(data: InboxDefaultViewV1Query$data | null): SafeHTMLString | null {
  if (!data || !data.node) return null
...
const notificationDefaultViewQuery = graphql`
  query InboxDefaultViewV1Query($id: ID!) {
    node(id: $id) {
...
export const InboxDefaultView = ({notificationId}: InboxDefaultViewProps) => {
  const [queryReference, loadQuery, disposeQuery] = useQueryLoader<InboxDefaultViewV1Query>(InboxDefaultViewRequest)
  useEffect(() => {
...
const InboxDefaultViewRenderer = ({queryReference}: InboxDefaultViewRendererProps) => {
  const data = usePreloadedQuery<InboxDefaultViewV1Query>(notificationDefaultViewQuery, queryReference)
