...
import {PullRequestCommitsViewer} from '../components/PullRequestCommitsViewer'
...
import type {PullRequestCommitsPageProps} from '../types'

export const PullRequestCommitsPage: EntryPointComponent<PullRequestCommitsPageProps, Record<string, never>> = ({
  queries,
}) => {
  const {
  ...
  const analyticsMetadata = useMemo(
    () => ({pullRequestId: itemIdentifier?.number.toString() ?? '', view: 'commits'}),
    [itemIdentifier?.number],
  )

  if (!itemIdentifier) throw new Error('no item identifier!')

  const {headerQuery, mainContentAreaQuery, contentQuery, deferredCommitsDataQuery} = queries
    ...
    <PageDataContextProvider basePageDataUrl={basePageDataUrl}>
      <AnalyticsProvider appName="pull_request" category="prx" metadata={analyticsMetadata}>
        <PullRequestLayout
          ...
          showFileTree={false}
          body={
            <PullRequestCommitsViewer
              {...itemIdentifier}
              pullRequestNumber={itemIdentifier.number}
              queries={{contentQuery, deferredCommitsDataQuery}}
            />
          }
        />
      </AnalyticsProvider>
