...
function getNotificationContent(thread: InboxListRow_fragment$data) {
  const {list} = thread
...
const InboxRowHierarchy: FC<InboxListRow_fragment$data> = memo(function InboxHierarchy(thread) {
  const {list} = thread
  const {__typename} = list
...
const InboxRowCompactHierarchy: FC<InboxListRow_fragment$data> = memo(function InboxRowCompactHierarchy(thread) {
  const {list} = thread
  const {__typename} = list
