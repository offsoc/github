...
module Seeds
  class Runner
    class PagesReposCodespaces < Seeds::Runner
      ...
      class << self
        def run(options = {})
          ...
          org = Seeds::Objects::Organization.create(login: ORG, admin: Seeds::Objects::User.monalisa)
        ...
        def copy_repo(owner_name, repo_name)
          ...
          pages_dir = Pathname.new(repo_name).join(".git")
          ...
          if Rails.env.test?
            ...
            repo = Seeds::Objects::Repository.restore_premade_repo(location_premade_git: pages_dir.realpath.to_s, owner_name: owner_name, repo_name: repo_name, is_public: true)
          end
        end
      end
    end
